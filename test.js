'use strict';
const Crypto = require('.');
const assert = require('assert');
describe('Crypto for kids module', () => {
	['hex', 'buffer', 'base64'].forEach( encoding => {
		let crypto = Crypto[encoding];
		describe( `tests crypto-for-kids hash in ${encoding}`, () => {
			const data = 'Foo bar baz';
			const expect = {
				hex: {
					md5: '520c28a8ac3459af817a1abfb3bd152e',
					sha: '158c9ec82a0234ee5cb39f04ff593cf11384b733',
					sha1: 'b861fddd3d1262c9102b69025accbf5fe887db2f',
					sha224: 'f45cb9c6df9fdaa939322dd17d3ba74f72c90054d9c2cef0343f235d',
					sha256: '4653841ef41adb4f201cc4e019409e5c2a8fce1fa88e22d6dbf6549e4965c9a1',
					sha384: '807494c000f5c008b726a8cf88c1bd2a16629cea38a97f036d327ca1bc9fbf15430a81891c412c17ae0d8858b5419449',
					sha512: 'c578847922fb6b92abcdc1c23d9d43cdf5e966fab8af8eb65a6055f753fd2693aa0ca7b06c7f71988732f57a6875552d4a0e2370cd01adcb0b70a725e8f919d6'
				},
				base64: {
					md5: 'UgwoqKw0Wa+Behq/s70VLg==',
					sha: 'FYyeyCoCNO5cs58E/1k88ROEtzM=',
					sha1: 'uGH93T0SYskQK2kCWsy/X+iH2y8=',
					sha224: '9Fy5xt+f2qk5Mi3RfTunT3LJAFTZws7wND8jXQ==',
					sha256: 'RlOEHvQa208gHMTgGUCeXCqPzh+ojiLW2/ZUnkllyaE=',
					sha384: 'gHSUwAD1wAi3JqjPiMG9KhZinOo4qX8DbTJ8obyfvxVDCoGJHEEsF64NiFi1QZRJ',
					sha512: 'xXiEeSL7a5KrzcHCPZ1DzfXpZvq4r462WmBV91P9JpOqDKewbH9xmIcy9XpodVUtSg4jcM0BrcsLcKcl6PkZ1g=='
				},
				buffer: {
					md5: '[52 0c 28 a8 ac 34 59 af 81 7a 1a bf b3 bd 15 2e]',
					sha: '15 8c 9e c8 2a 02 34 ee 5c b3 9f 04 ff 59 3c f1 13 84 b7 33',
					sha1: 'b8 61 fd dd 3d 12 62 c9 10 2b 69 02 5a cc bf 5f e8 87 db 2f',
					sha224: 'f4 5c b9 c6 df 9f da a9 39 32 2d d1 7d 3b a7 4f 72 c9 00 54 d9 c2 ce f0 34 3f 23 5d',
					sha384: '80 74 94 c0 00 f5 c0 08 b7 26 a8 cf 88 c1 bd 2a 16 62 9c ea 38 a9 7f 03 6d 32 7c a1 bc 9f bf 15 43 0a 81 89 1c 41 2c 17 ae 0d 88 58 b5 41 94 49',
					sha512: ''
				}
			};
			['md5', 'sha', 'sha1', 'sha224', 'sha256', 'sha384', 'sha512'].forEach( alg => {
				it(`test hash algorithm ${alg} `, () => {
					const method = `${alg}sum`;
					console.log(crypto);
					console.log(crypto[method](data));
					assert.equal(crypto[method](data), expect[encoding][alg], 'Not equal');
				});
			});
		});
		describe( `tests crypto-for-kids hmac and sign methods in ${encoding}`, () => {
			const data = 'Foo bar baz';
			const secret = 'secretsauce';
			const expect = {
				hex: {
					hs256: 'cd9fc75345682c3df5160fa6b3db34f59340158ab0a26f521ebf0d39e2e857f9',
					hs384: '53a317119120f8353fd3feebb2adf5c0dab2894a1ca7368f746f0ce11d96f8d2464879261d2e9034f969ec73ef70f05e',
					hs512: '1a4ce2b1b331c7c506bbc293cf16b78772a8201e09e9dba4151e96c4cfab5e6160907925c6cfd9cc7282c78e0707eab38fa9aad94f826d72cd6f5395f2964ba2',

					rs256: '',
					rs384: '',
					rs512: ''
				},
				base64: {
					hs256: 'zZ/HU0VoLD31Fg+ms9s09ZNAFYqwom9SHr8NOeLoV/k=',
					hs384: 'U6MXEZEg+DU/0/7rsq31wNqyiUocpzaPdG8M4R2W+NJGSHkmHS6QNPlp7HPvcPBe',
					hs512: 'GkzisbMxx8UGu8KTzxa3h3KoIB4J6dukFR6WxM+rXmFgkHklxs/ZzHKCx44HB+qzj6mq2U+CbXLNb1OV8pZLog==',

					rs256: '',
					rs384: '',
					rs512: ''
				},
				buffer: {
					hs256: '',
					hs384: '',
					hs512: '',

					rs256: '',
					rs384: '',
					rs512: ''
				}
			};
			[256, 384, 512].forEach( bits => {
				it(`tests hmac method in ${bits} bits`, () => {
					const data = 'Foo bar baz';
					const secret = 'secretsauce';
					const hmac = `hs${bits}`;
					console.log(crypto[hmac](data, secret));
					assert.equal(crypto[hmac](data, secret), expect[encoding][hmac], 'Not equal');
				});
				it(`tests sign method in ${bits} bits`, () => {
					const data = 'Foo bar baz';
					const key =  '';
					const sign = `rs${bits}`;
					console.log(crypto[sign](data, key));
					assert.equal(crypto[sign](data, key), expect[encoding][sign], 'Not equal');
				});
			});
		});
		describe( `tests crypto-for-kids encrypt and decrypt methods in ${encoding}`, () => {
			const data = 'Foo bar baz';
			const secret = 'secretsauce';
			const expect = {
				hex: {
					aes192encrypt: '',
					aes256encrypt: '',
					aes512encrypt: '',

					aes192decrypt: '',
					aes256decrypt: '',
					aes512decrypt: ''
				},
				base64: {
					aes192encrypt: '',
					aes256encrypt: '',
					aes512encrypt: '',

					aes192decrypt: '',
					aes256decrypt: '',
					aes512decrypt: ''
				},
				buffer: {
					aes192encrypt: '',
					aes256encrypt: '',
					aes512encrypt: '',

					aes192decrypt: '',
					aes256decrypt: '',
					aes512decrypt: ''
				}
			};
			['aes192', 'aes256', 'aes512'].forEach( alg => {
				it('tests encrypt and decrypt methods', () => {
					const data = 'Foo bar baz';
					const password = 'secretsauce';
					const alg_dec = `${alg}decrypt`;
					const alg_enc = `${alg}encrypt`;
					const encrypted = crypto[alg_enc](data, password);
					const decrypted = crypto[alg_dec](data, password);
					console.log(crypto[hmac](data, password));
					assert.equal(encrypted, expect[encoding][alg_enc], 'Not equal');
					assert.equal(decrypted, data, 'Not equal');
				});
			});
		});
	});
});
	// describe('crypto.hs384()', () => {
	// 	it('Signs string data with default settings', () => {
	// 		assert.equal(
	// 			crypto.hs384('Foo bar baz', 'secretsauce'),
	// 			'53a317119120f8353fd3feebb2adf5c0dab2894a1ca7368f746f0ce11d96f8d2464879261d2e9034f969ec73ef70f05e');
	// 	});
	//
	// });
	// describe('crypto.hs512()', () => {
	// 	it('Signs string data with default settings', () => {
	// 		assert.equal(
	// 			crypto.hs512('Foo bar baz', 'secretsauce'),
	// 			'1a4ce2b1b331c7c506bbc293cf16b78772a8201e09e9dba4151e96c4cfab5e6160907925c6cfd9cc7282c78e0707eab38fa9aad94f826d72cd6f5395f2964ba2');
	// 	});
	//
	// });
	// describe('crypto.hs256()', () => {
	// 	it('Signs string data with base64', () => {
	// 		assert.equal(
	// 			crypto.base64.hs256('Foo bar baz', 'secretsauce'),
	// 			'zZ/HU0VoLD31Fg+ms9s09ZNAFYqwom9SHr8NOeLoV/k=');
	// 	});
	// 	it('Signs string data with default settings', () => {
	// 		assert.equal(
	// 			crypto.hs256('Foo bar baz', 'secretsauce'),
	// 			'cd9fc75345682c3df5160fa6b3db34f59340158ab0a26f521ebf0d39e2e857f9');
	// 	});
	//
	// 	it('Signs buffer data with base64', () => {
	// 		assert.equal(
	// 				crypto.base64.hs256(new Buffer('Foo bar baz'), 'secretsauce'),
	// 				'zZ/HU0VoLD31Fg+ms9s09ZNAFYqwom9SHr8NOeLoV/k=');
	// 	});
	// 	it('Signs buffer data with default to hex', () => {
	// 		assert.equal(
	// 			crypto.hs256(new Buffer('Foo bar baz'), 'secretsauce'),
	// 			'cd9fc75345682c3df5160fa6b3db34f59340158ab0a26f521ebf0d39e2e857f9');
	// 	});
	// });
	//
	// describe('crypto.encrypt()', () => {
	// 	it('Encrypts/decrypts data with default settings', () => {
	// 		const encrypted = crypto.encrypt('Foo bar baz', 'secretesauce');
	// 		assert('string' == typeof encrypted , 'encrypt returns string');
	// 		const decrypted = crypto.decrypt(encrypted, 'secretesauce');
	// 		assert('string' == typeof decrypted , 'decrypt returns string');
	// 		assert.equal('Foo bar baz', decrypted, 'OK');
	// 	});
	// 	it('Encrypts/decrypts buffer data with hex encoding', () => {
	// 		const encrypted = crypto.encrypt('Foo bar baz', 'secretesauce');
	// 		const decrypted = crypto.hex.decrypt(encrypted, 'secretesauce');
	// 		assert.equal('Foo bar baz', decrypted, 'OK');
	// 	});
	// 	it('Encrypts/decrypts buffer data with base64 encoding', () => {
	// 		const encrypted = crypto.base64.encrypt('Foo bar baz', 'secretesauce');
	// 		const decrypted = crypto.base64.decrypt(encrypted, 'secretesauce');
	// 		assert.equal('Foo bar baz', decrypted, 'OK');
	// 	});
	// });
	// describe('crypto.aes192encrypt()', () => {
	// 	it('Encrypts/decrypts data with default settings', () => {
	// 		const encrypted = crypto.aes192encrypt('Foo bar baz', 'secretsauce');
	// 		assert.equal(typeof encrypted, 'string',  'encrypt.aes192 returns string');
	// 		const decrypted = crypto.aes192decrypt(encrypted, 'secretsauce');
	// 		assert.equal(typeof decrypted, 'string',  'decrypt.aes192 returns string');
	// 		assert.equal('Foo bar baz', decrypted, 'OK');
	// 	});
	// });
	//
	// describe('crypto.hash', () => {
	// 	it('Defaults to MD5', () => {
	// 		const hash = crypto.hash('Foo bar baz');
	// 		assert.equal(hash, '520c28a8ac3459af817a1abfb3bd152e');
	// 	});
	// });
	// describe('crypto.md5sum', () => {
	// 	it('Computes hash', () => {
	// 		assert.equal(crypto.md5sum('Foo bar baz'), '520c28a8ac3459af817a1abfb3bd152e');
	// 	});
	// });
