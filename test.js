	...
	['hex', 'buffer', 'base64'].forEach( encoding => {
		...
		describe( `tests crypto-for-kids hmac and sign methods in ${encoding}`, () => {
			...
			const expect = {
				...
				private_keys: {
					rs256: fs('./data/key256.pem'),
					rs384: fs('./data/key384.pem'),
					rs512: fs('./data/key512.pem')
				},
				public_keys: {
			...
			[256, 384, 512].forEach( bits => {
				it(`tests hmac method in ${bits} bits`, () => {
					...
					if  (encoding === 'buffer') {
						assert.ok((crypto[hmac](data, secret).compare(expect[encoding][hmac])) === 0, 'Buffers not equal');
					} else {
				...
				it(`tests sign method in ${bits} bits`, () => {
					const data = 'Foo bar baz';
					const sign = `rs${bits}`;
					const key =  expect.private_keys[sign].toString();
